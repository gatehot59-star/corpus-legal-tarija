...
Synthetic-only loopback acceptance. It never sends real mail, reaches external
URLs, edits source, or deploys. CI provisions Fontconfig and runs headed under Xvfb.
"""
...
def run_manage(*args: str) -> str:
    """Run a management command and fail with complete output on error."""
...
def wait_live(base: str) -> None:
    """Wait for the real WSGI process before launching browser checks."""
...
def main() -> None:
    """Execute desktop, mobile, ownership, recovery and withdrawal checks."""
    run_manage("migrate", "--noinput")
    fixture = run_manage("provision_fixture", "--snapshot", str(DB.with_name("snapshot.sqlite3")))
    version_match = re.search(r'"version": "([0-9a-f]{64})"', fixture)
    assert version_match, fixture
    version = version_match.group(1)
    with tempfile.TemporaryDirectory(prefix="corpus-browser-mail-") as maildir:
        with subprocess.Popen([
            PYTHON, "-c",
            "from django.conf import settings; "
            "settings.CORPUS_ORIGIN='http://127.0.0.1:8000'; "
