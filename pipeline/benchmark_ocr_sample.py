...
manifest entries across rubro and gestion, downloads source PDFs, samples first/
middle/last pages, and runs the pinned Tesseract and PaddleOCR candidates on
identical 300 dpi grayscale images.
"""
...
def sample_rows(manifest, n_docs):
    ...
    for line in manifest.open(encoding="utf-8"):
        ...
        if not r.get("fuente_url") or not r.get("paginas"): continue
        rows.append(r)
    rows.sort(key=lambda r:(r.get("rubro",""),r.get("gestion",""),r.get("sha256","")))
    buckets={}
...
def pages_for(total):
    vals=sorted(set([1,max(1,(total+1)//2),total]))
    return vals

def run_tess(image,dst,tessdata):
    dst.parent.mkdir(parents=True,exist_ok=True); env=dict(os.environ,TESSDATA_PREFIX=str(tessdata))
    t=time.monotonic(); cp=subprocess.run(["tesseract",str(image),str(dst),"-l","spa","--psm","3"],env=env,capture_output=True,text=True)
...
def main():
    ...
    for ix,r in enumerate(selected,1):
        ...
        pages=pages_for(int(r["paginas"])); prefix=images/key; prefix.mkdir(parents=True,exist_ok=True)
        for page in pages:
            out=prefix/f"page-{page:04d}"; subprocess.run(["pdftoppm","-f",str(page),"-l",str(page),"-r",str(args.dpi),"-gray","-png",str(pdf),str(out)],check=True,capture_output=True)
            png=sorted(prefix.glob(out.name+"-*.png"))[0]; rows.append({"sample_index":ix,"sha256":key,"title":r.get("nombre_servidor"),"rubro":r.get("rubro"),"gestion":r.get("gestion"),"page":page,"pages_total":r.get("paginas"),"image":str(png),"source_url":r.get("fuente_url")})
    ...
    for label,tessdata in (("tesseract_fast_psm3",args.tessdata_fast),("tesseract_best_psm3",args.tessdata_best)):
        out=[]
        for row in rows:
            res=run_tess(Path(row["image"]),args.output/label/(row["sha256"]+f"-p{row['page']:04d}"),tessdata); out.append({**row,**{k:v for k,v in res.items() if k!="text"}}); (args.output/label/(row["sha256"]+f"-p{row['page']:04d}.txt")).write_text(res["text"],encoding="utf-8")
        report["engines"][label]=out
    from paddleocr import PaddleOCR
    engine=PaddleOCR(lang="es",ocr_version="PP-OCRv5",text_detection_model_name="PP-OCRv5_mobile_det",text_recognition_model_name="latin_PP-OCRv5_mobile_rec",device="cpu",enable_mkldnn=False,use_doc_orientation_classify=False,use_doc_unwarping=False,use_textline_orientation=False)
    out=[]
    for row in rows:
        ...
        text="\n".join(texts); (args.output/"paddleocr"/(row["sha256"]+f"-p{row['page']:04d}.txt")).write_text(text,encoding="utf-8")
        out.append({**row,"seconds":round(time.monotonic()-t,3),"chars":len(text),"terms":terms(text),"gate":gate(text)})
